...
class GraduateService(
    ...
    private fun extractGraduateInfoFromCSV(isDoctorateGraduate: Boolean, line: List<String>): ResponseResult<Nothing?> {
        ...
        var createdUser = when (val result = userService.findByEmail(fields.getValue(CsvFieldsEnum.EMAIL))) {
            is ResponseResult.Success -> result.data
        ...
        if (createdUser == null) {
            val userDTO = RegisterDTO(
                ...
                email = fields.getValue(CsvFieldsEnum.EMAIL),
            ...
            val graduate = Graduate(
                user = createdUser,
                hasFinishedDoctorateOnUFF = fields.getValue(CsvFieldsEnum.FINISHED_DOCTORATE_ON_UFF).toBoolean(),
                hasFinishedMasterDegreeOnUFF = fields.getValue(CsvFieldsEnum.FINISHED_MASTER_DEGREE_ON_UFF).toBoolean(),
        ...
        if (postDoctorateFieldValue.isNotEmpty()) {
            ...
            val postDoctorate = when (val result = postDoctorateService.createPostDoctorate(createdUser, postDoctorateDTO)) {
                is ResponseResult.Success -> result.data!!
                is ResponseResult.Error -> return ResponseResult.Error(Errors.INVALID_DATA)
            }
        }
        ...
        val advisor = when (val result = advisorService.findAdvisorByName(fields.getValue(CsvFieldsEnum.ADVISOR_NAME))){
            is ResponseResult.Success -> result.data!!
            is ResponseResult.Error -> return ResponseResult.Error(Errors.INVALID_DATA)
        }

        val ciProgram = when (val result = ciProgramService.findProgramByInitials(fields.getValue(CsvFieldsEnum.PROGRAM_NAME))){
            is ResponseResult.Success -> result.data!!
            is ResponseResult.Error -> return ResponseResult.Error(Errors.INVALID_DATA)
        }
