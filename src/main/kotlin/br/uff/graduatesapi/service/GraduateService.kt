...
class GraduateService(
		...
		return when (val result = userService.findByEmail(fields.email)) {
			...
			is ResponseResult.Error -> {
				...
				val createdUser = when (val resultCreatedUser = userService.createUser(userDTO)) {
					...
					is ResponseResult.Error -> return ResponseResult.Error(resultCreatedUser.errorReason)
				}
				...
				when (val resultCreatedGraduate = createGraduateByModel(graduate)) {
					...
					is ResponseResult.Error -> ResponseResult.Error(resultCreatedGraduate.errorReason)
				}
			}
		}
	}
	...
	private fun handlePostDoctorateFromCSV(
		...
		return when (
			...
			is ResponseResult.Error -> ResponseResult.Error(Errors.INVALID_DATA)
		}
	}
	...
	private fun extractGraduateInfoFromCSV(
		...
		val course = when (val result = handleCourseFromCSV(graduate, csvFields)) {
			is ResponseResult.Success -> result.data!!
			is ResponseResult.Error -> return ResponseResult.Error(result.errorReason)
		}

		val postDoctorate = when (val result = handlePostDoctorateFromCSV(csvFields.postDoctorate, graduate)) {
			is ResponseResult.Success -> result.data
			is ResponseResult.Error -> return ResponseResult.Error(result.errorReason)
		}

		val cnpqScholarship = when (val result = handleCNPQScholarshipFromCSV(graduate, csvFields)) {
			is ResponseResult.Success -> result.data
			is ResponseResult.Error -> return ResponseResult.Error(result.errorReason)
		}

		val workHistory = when (val result = handleWorkHistoryFromCSV(graduate, csvFields)) {
			is ResponseResult.Success -> result.data
			is ResponseResult.Error -> return ResponseResult.Error(result.errorReason)
		}
	...
	fun createGraduateByCSV(file: MultipartFile, isDoctorateGraduates: Boolean): ResponseResult<Nothing?> {
		...
		for (line in lines) {
			...
			if (extractResult is ResponseResult.Error) {
				...
				return ResponseResult.Error(extractResult.errorReason)
			}
		}
		return ResponseResult.Success(null)
	}
